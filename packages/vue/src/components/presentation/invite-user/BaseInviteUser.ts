  ...
  setup(props, {slots}) {
    return () => {
      ...
      return h(
        'div',
        {
          class: [withVendorCSSClassPrefix('invite-user--coming-soon'), props.className].filter(Boolean).join(' '),
          style: 'padding: 24px; border: 1px dashed #ccc; border-radius: 8px; text-align: center;',
        },
        [
          h('h3', {style: 'margin: 0 0 8px 0;'}, 'Invite User'),
          h('p', {style: 'color: #666; margin: 0; font-size: 14px;'}, 'Coming Soon — This embedded user invitation component will be available when app-native authentication flow is implemented.'),
        ],
      );
    };
  },
