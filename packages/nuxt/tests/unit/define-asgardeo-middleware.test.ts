...
describe('defineAsgardeoMiddleware', () => {
  describe('unauthenticated user', () => {
    it('redirects to /api/auth/signin by default', async () => {
      const middleware = defineAsgardeoMiddleware();
    ...
    it('honours a custom redirectTo', async () => {
      const middleware = defineAsgardeoMiddleware({redirectTo: '/login'});
  ...
  describe('authenticated user', () => {
    ...
    it('allows access when required scopes are present', async () => {
      const middleware = defineAsgardeoMiddleware({requireScopes: ['openid', 'profile']});
      await (middleware as Function)(makeTo('/dashboard'), makeTo('/'));
      expect(_navigateToTarget).toBeUndefined();
    });

    it('redirects when a required scope is missing', async () => {
      const middleware = defineAsgardeoMiddleware({requireScopes: ['admin']});
  ...
  describe('AsgardeoMiddlewareOptions type', () => {
    it('accepts an empty options object', () => {
      const opts: AsgardeoMiddlewareOptions = {};
