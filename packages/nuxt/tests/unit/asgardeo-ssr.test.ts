...
vi.mock('#imports', () => ({
  useRuntimeConfig: vi.fn(() => ({
    public: {asgardeo: {baseUrl: 'https://api.asgardeo.io/t/testorg', preferences: undefined}},
  })),
}));

vi.mock('../../src/runtime/server/AsgardeoNuxtClient', () => ({
  default: {
...
vi.mock('../../src/runtime/server/utils/session', () => ({
  ...
  getSessionCookieName: vi.fn(() => 'asgardeo-session'),
}));
...
describe('asgardeo-ssr Nitro plugin', () => {
  ...
  it('returns early for /api/ paths without setting asgardeo context', async () => {
    const event = await callHandler('/api/auth/session');
    expect(event.context.asgardeo).toBeUndefined();
  });
  ...
  it('sets isSignedIn:false when no session cookie is present', async () => {
    ...
    expect(event.context.asgardeo).toEqual({session: null, isSignedIn: false});
  });
  ...
  it('writes legacy __asgardeoAuth to event context for backwards compatibility', async () => {
    const event = await callHandler('/', 'valid-cookie');

    expect(event.context.__asgardeoAuth).toBeDefined();
    expect(event.context.__asgardeoAuth.isSignedIn).toBe(true);
    expect(event.context.__asgardeoAuth.user).toEqual({sub: 'user-123', email: 'test@example.com'});
  });
  ...
  it('skips getUserProfile when preferences.user.fetchUserProfile is false', async () => {
    ...
    expect(event.context.asgardeo.ssr.userProfile).toBeNull();
    // other fields should still be populated
    expect(event.context.asgardeo.ssr.myOrganizations).toHaveLength(1);
    expect(event.context.asgardeo.ssr.brandingPreference).toBeDefined();
  });
  ...
  it('skips org fetches when preferences.user.fetchOrganizations is false', async () => {
    ...
    expect(event.context.asgardeo.ssr.myOrganizations).toEqual([]);
    expect(event.context.asgardeo.ssr.currentOrganization).toBeNull();
    // user and branding should still be populated
    expect(event.context.asgardeo.ssr.user).toBeDefined();
    expect(event.context.asgardeo.ssr.brandingPreference).toBeDefined();
  });
  ...
  it('skips branding fetch when preferences.theme.inheritFromBranding is false', async () => {
    ...
    expect(event.context.asgardeo.ssr.brandingPreference).toBeNull();
    // other fields should still be populated
    expect(event.context.asgardeo.ssr.user).toBeDefined();
    expect(event.context.asgardeo.ssr.myOrganizations).toHaveLength(1);
  });
  ...
  it('still writes SSR data when getUserProfile throws (non-fatal)', async () => {
    ...
    expect(event.context.asgardeo.ssr).toBeDefined();
    expect(event.context.asgardeo.ssr.isSignedIn).toBe(true);
    expect(event.context.asgardeo.ssr.userProfile).toBeNull();
    // user fetch ran independently and should succeed
    expect(event.context.asgardeo.ssr.user).toEqual({sub: 'user-123', email: 'test@example.com'});
  });
