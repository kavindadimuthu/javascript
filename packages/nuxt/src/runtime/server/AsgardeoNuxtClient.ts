...
class AsgardeoNuxtClient extends AsgardeoNodeClient<AsgardeoNuxtConfig> {
  ...
  async rehydrateSessionFromPayload(session: AsgardeoSessionPayload): Promise<void> {
    ...
    const storageManager: Awaited<ReturnType<typeof this.legacy.getStorageManager>> =
      await this.legacy.getStorageManager();
  ...
  override signIn(...args: any[]): Promise<any> {
    ...
    if (typeof arg0 === 'object' && arg0 !== null && 'flowId' in arg0) {
      ...
      if ((arg0 as {flowId: string}).flowId === '') {
  ...
  override async getUserProfile(sessionId: string): Promise<UserProfile> {
    ...
    try {
      const authHeaders: {Authorization: string} = {Authorization: `Bearer ${accessToken}`};
   ...
   */
  static async getBrandingPreference(config: GetBrandingPreferenceConfig): Promise<BrandingPreference> {
