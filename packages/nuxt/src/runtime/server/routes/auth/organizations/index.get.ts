 ...
 */
export default defineEventHandler(async (event: H3Event): Promise<AllOrganizationsApiResponse> => {
  ...
  try {
    ...
    return await client.getAllOrganizations(undefined, session.sessionId);
  } catch (err) {
    throw createError({
      statusCode: 500,
