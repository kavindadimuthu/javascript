...
export default defineNuxtModule<AsgardeoNuxtConfig>({
  ...
  setup(userOptions: AsgardeoNuxtConfig, nuxt: Nuxt) {
    ...
    Object.assign(nuxt.options.runtimeConfig, {
      asgardeo: defu((nuxt.options.runtimeConfig.asgardeo as Record<string, unknown>) || {}, privateConfig) as {
        clientSecret: string;
    ...
    Object.assign(nuxt.options.runtimeConfig.public, {
      asgardeo: defu((nuxt.options.runtimeConfig.public.asgardeo as Record<string, unknown>) || {}, {
    ...
    const serverRoutes: Array<{handler: string; method?: 'post' | 'patch'; route: string}> = [
      // ── Auth flow ──────────────────────────────────────────────────────
      ...
      // ── Organisations ─────────────────────────────────────────────────
      {handler: resolve('./runtime/server/routes/auth/organizations/index.get'), route: '/api/auth/organizations'},
      {
        handler: resolve('./runtime/server/routes/auth/organizations/index.post'),
        method: 'post' as const,
        route: '/api/auth/organizations',
      },
      {handler: resolve('./runtime/server/routes/auth/organizations/me.get'), route: '/api/auth/organizations/me'},
      {
        handler: resolve('./runtime/server/routes/auth/organizations/current.get'),
        route: '/api/auth/organizations/current',
      },
      {handler: resolve('./runtime/server/routes/auth/organizations/id.get'), route: '/api/auth/organizations/:id'},
      {
    ...
    serverRoutes.forEach((sr: {handler: string; method?: 'post' | 'patch'; route: string}) => {
